...
| `memory-embeddings` | ok / warn | Every live memory entry carries a vector. Entries stored while the embedding provider was unavailable are invisible to semantic search; `warn` reports the count and points at `mc-tool-memory reindex`. |
