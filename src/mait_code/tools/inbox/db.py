"""Shared database connection factory for the inbox tool.

All inbox modules should import ``get_connection()`` / ``connection()`` from
...
def get_project() -> str:
    ...
    current directory name. On the inbox this is only a capture-context hint —
    the store itself is global.
    """
