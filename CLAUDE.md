...
- **New skill:** Create directory in `skills/` with `SKILL.md` — skills can invoke CLI tools via preprocessing or Bash. Grant the narrowest `allowed-tools` covering the commands the skill actually runs. Measured against Claude Code 2.1.220: `Bash(cmd)` is an exact match; `Bash(cmd:*)` is a prefix match **at a token boundary**; `Bash(cmd *)` (the older space form) is a real wildcard that behaved identically to `cmd:*` on every case tested — so `Bash(git *)` permits `git push` and `Bash(mc-tool-board *)` permits `mc-tool-board remove`. Everything after the boundary is permitted, flags included, so `Bash(git branch:*)` does span `git branch -D`; narrow inside the prefix (`Bash(git diff --stat:*)`) or pin to an exact invocation. `:*` does **not** span a longer token — `Bash(git diff:*)` refuses `git difftool` — so the old "raw prefix, no word boundary" claim is false on this version. A grant buys **unsandboxed** execution, not merely a quiet prompt: an ungranted command that Claude Code can contain runs in a filesystem-isolated sandbox with no prompt and its writes are discarded (`touch marker.txt` created nothing under an unrelated rule, created the file under `Bash(touch:*)`), while uncontainable ones (`git commit`, `git push`) are refused outright. So narrowing a grant need not break a skill — read-only commands keep working sandboxed — and "no prompt appeared" is not evidence a rule matched. Claude Code is also operator-aware: `Bash(git push:*)` refuses `git push --dry-run || git commit --allow-empty -m x`. Verify with `perms.matches_command` against `perms.MUTATING_INVOCATIONS` — a floor, not a proof. All of this is undocumented matcher behaviour pinned to 2.1.220; prefer prefixes that stay safe if it changes
